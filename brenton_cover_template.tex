...
% Spine:
\newdimen\SpineW   \SpineW=1.86in

% Bleed (Snowfall Press typical: 0.125in):
\newdimen\Bleed    \Bleed=0.125in

% Title text:
\newcommand{\CoverTitle}{Η ΠΑΛΑΙΑ ΔΙΑΘΗΚΗ}
\newcommand{\SubtitleEnglish}{The Septuagint}
\newcommand{\SubtitleGreek}{Brenton's Greek Text}
...
\begin{tikzpicture}[x=1in,y=1in]
  ...
  % Brenton attribution (smaller)
  \node[align=center, text=CoverText] at (\FrontCenterX, \FrontTitleY - 1.0in)
    {\textenglish{\fontsize{\FrontSubtitleSmallSize}{18pt}\selectfont \SubtitleGreek}};

  % --- SPINE TITLE (HORIZONTAL) ---
  % Position slightly higher to align with front title (compensating for font size difference)
  \newdimen\SpineTitleY
  \SpineTitleY=\dimexpr\FrontTitleY \relax
  \node[align=center, text=CoverText] at (\SpineCenterX, \SpineTitleY)
    {{\fontsize{16pt}{19pt}\selectfont Η ΠΑΛΑΙΑ} \\[6pt] {\fontsize{16pt}{19pt}\selectfont ΔΙΑΘΗΚΗ}};
  
  % --- SPINE SUBTITLES (HORIZONTAL) ---
  \node[align=center, text=CoverText] at (\SpineCenterX, \SpineTitleY - 0.5in)
    {\textenglish{\fontsize{\SpineSubtitleSize}{12pt}\selectfont \SubtitleEnglish}};
  
  % Brenton attribution on spine
  \node[align=center, text=CoverText] at (\SpineCenterX, \SpineTitleY - 1in)
    {\textenglish{\fontsize{8pt}{10pt}\selectfont\baselineskip=10pt Brenton's Greek Text}};
